...
import { Users, TrendingUp, Award, Search, X, Loader2, RefreshCw } from 'lucide-react'
...
export default function SellerGrid({ onSelectSeller }: SellerGridProps) {
  ...
  useEffect(() => {
    ...
    // Polling de conexões a cada 30s para detectar login/logout em tempo real
    const connectionsInterval = setInterval(refreshConnections, 30000)
  ...
  // Refresh apenas conexões (leve, sem recarregar tudo)
  const refreshConnections = async () => {
    try {
      ...
      const res = await fetch('/api/admin/seller-connections', {
        headers: { 'x-company-id': companyId }
      })
      const data = await res.json()
      if (data.data) {
        setSellerConnections(data.data)
      }
    } catch {
      // Silencioso — polling não deve mostrar erros
    }
  }
  ...
  // Loading state — skeleton cards instead of empty state flash
  if (loading) {
    return (
      <div>
    ...
    <div>
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <button
          ...
          disabled={refreshing}
          className="p-2.5 bg-white border border-gray-200 rounded-xl text-gray-500 hover:text-green-600 hover:border-green-300 transition-all disabled:opacity-50"
                ...
                <div className="flex items-center gap-3 mt-3 pt-3 border-t border-gray-100">
                  ...
                  <div className="flex items-center gap-2 ml-auto">
                    ...
                    <div className="relative">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center ${
                          conn.whatsapp ? 'bg-white ring-2 ring-green-400' : 'bg-gray-100'
                        }`}
                        title={conn.whatsapp ? 'WhatsApp conectado — online agora' : 'WhatsApp desconectado'}
                      >
