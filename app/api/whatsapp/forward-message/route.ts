...
export async function POST(request: NextRequest) {
  try {
    const { waMessageId, targetChatId } = await request.json()

    if (!waMessageId || !targetChatId) {
      return NextResponse.json({ error: 'waMessageId and targetChatId are required' }, { status: 400 })
    }
    ...
    await Promise.race([
      msg.forward(targetChatId),
      new Promise<never>((_, reject) => setTimeout(() => reject(new Error('Timeout ao reencaminhar mensagem')), 15000))
    ])
    ...
    return NextResponse.json(
      { error: error.message || 'Erro ao reencaminhar mensagem' },
      { status: 500 }
    )
  }
}
