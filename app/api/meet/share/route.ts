...
function buildSmartNotesHtml(notes: any): string {
...
function buildTranscriptHtml(transcript: any[]): string {
  if (!transcript?.length) return ''
  // Limit to 60 segments max for email size
...
function buildEvaluationHtml(evaluation: any): string {
...
function buildSpinHtml(evaluation: any, spinScores: { s: number | null, p: number | null, i: number | null, n: number | null }): string {
  let html = ''
  const scores = [
  ...
  for (const item of scores) {
    ...
    // Add technical feedback from evaluation if available
...
function buildShareEmailHtml(
  ...
  // Build content blocks for each shared section
  let contentBlocks = ''
